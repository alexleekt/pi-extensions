...
import { run as runTopic, suiteFactory as topicSuiteFactory } from "./suites/topic.suite.js";
import { run as runGoal, suiteFactory as goalSuiteFactory } from "./suites/goal.suite.js";
...
async function main() {
  ...
  if (suite === "optimize") {
    ...
    if (!subSuite || !promptFile || !desirableOutcome || Number.isNaN(targetPassRate)) {
    ...
    const promptPath = promptFile.startsWith("/") ? promptFile : path.join(toolsDir, "..", promptFile);

    const factory = subSuite === "topic" ? topicSuiteFactory(testCasesFile) :
                    subSuite === "goal"  ? goalSuiteFactory(testCasesFile) :
                    null;
    ...
    const result = await optimizeSuite(factory as any, promptPath, {
      desirableOutcome,
      targetPassRate,
      evalModel: model,
      criticModel: model,
    ...
    console.log(`   Best pass rate: ${result.bestIteration.passRate.toFixed(1)}% (iteration ${result.bestIteration.iteration})`);
