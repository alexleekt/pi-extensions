...
const TEST_CASES: TestCase[] = JSON.parse(
  fs.readFileSync(path.join(import.meta.dirname, "test-cases-achievement.json"), "utf8"),
);
...
function hasFilePath(text: string): boolean {
  return /\b(?:src\/|test\/|docs\/|lib\/|bin\/|\.github\/|\.gitignore|\.dockerignore|Dockerfile|\.ts|\.js|\.json|\.yml|\.md|\.py|\.rs|\.go|\.java|\.rb):?\b/.test(text);
}

function hasCommandOrCount(text: string): boolean {
  return /\b\d+\s+(?:files?|tests?|lines?|modules?|endpoints?|functions?|classes?|methods?|dependencies?|configs?|docs?|diagrams?)\b|\b(?:npm|yarn|pnpm|pip|cargo|go|cargo|docker|kubectl|terraform|ansible|make|cmake|gradle|maven|jest|pytest|mocha|vitest|playwright|cypress)\b/i.test(text);
}
...
function score(text: string, maxWords: number, expected: string, goal?: string): Score {
  ...
  const noMetaCommentary = !/^\s*(The user|User wants|asking a|giving feedback|I think|You should|Here is|This is)/i.test(extracted);
  const noQuotes = !/^["'`].*["'`]$/s.test(extracted.trim());
  const noMarkdown = !/```/.test(extracted);
  const validJson = isValidJson(text);
  const echoesGoal = goal !== undefined ? wordOverlap(goal, extracted) >= 0.25 : true;
  const specificConcrete = hasFilePath(extracted) || hasCommandOrCount(extracted);
  const alignsWithExpected = wordOverlap(expected, extracted) >= 0.3;
  const noVagueFiller = !hasVagueFiller(extracted);

  const total = [
    withinLimit, pastTense, noMetaCommentary, noQuotes, noMarkdown,
    validJson, echoesGoal, specificConcrete, alignsWithExpected, noVagueFiller,
  ].filter(Boolean).length;

  return {
    wordCount, withinLimit, pastTense, noMetaCommentary, noQuotes, noMarkdown,
    validJson, echoesGoal, specificConcrete, alignsWithExpected, noVagueFiller,
    total,
  };
}
...
function buildAchievementPrompt(instructions: string, maxWords: number, message: string, goal?: string): string {
...
async function evaluate(model: string): Promise<EvalResult[]> {
  ...
  for (const tc of TEST_CASES) {
    ...
    const raw = await callLLM(
      buildAchievementPrompt(instructions, tc.max_words, tc.message, tc.goal),
      tc.message,
      model,
...
function report(results: EvalResult[], model: string): string {
  ...
  for (const r of results) {
    ...
    lines.push(`### Input: "${preview}${r.message.length > 80 ? "…" : ""}"`);
    lines.push("");
    lines.push(`**Expected**: "${r.expected}"`);
    lines.push(`**Got** (${r.score.wordCount} words, score ${r.score.total}/10):`);
    lines.push(`\`\`\`text`);
    ...
    lines.push(`|-----------|------|`);
    lines.push(`| 1. Within word limit | ${r.score.withinLimit ? "✅" : "❌"} |`);
    lines.push(`| 2. Past tense | ${r.score.pastTense ? "✅" : "❌"} |`);
    lines.push(`| 3. No meta-commentary | ${r.score.noMetaCommentary ? "✅" : "❌"} |`);
    lines.push(`| 4. No wrapping quotes | ${r.score.noQuotes ? "✅" : "❌"} |`);
    lines.push(`| 5. No markdown fences | ${r.score.noMarkdown ? "✅" : "❌"} |`);
    lines.push(`| 6. Valid JSON format | ${r.score.validJson ? "✅" : "❌"} |`);
    lines.push(`| 7. Echoes goal terminology | ${r.score.echoesGoal ? "✅" : "❌"} |`);
    lines.push(`| 8. Specific / concrete | ${r.score.specificConcrete ? "✅" : "❌"} |`);
    lines.push(`| 9. Aligns with expected | ${r.score.alignsWithExpected ? "✅" : "❌"} |`);
    lines.push(`| 10. No vague filler | ${r.score.noVagueFiller ? "✅" : "❌"} |`);
    lines.push("");
  }
...
async function main() {
  ...
  const outFile = path.join(import.meta.dirname, `prompt-eval-achievement-report.md`);
