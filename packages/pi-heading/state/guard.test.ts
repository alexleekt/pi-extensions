...
describe("stableTopic", () => {
    ...
    test("keeps old topic on exact match (case insensitive)", () => {
        expect(stableTopic("Docker setup", "docker setup")).toBe(
            "Docker setup",
        );
        expect(stableTopic("docker setup", "Docker Setup")).toBe(
            "docker setup",
    ...
    test("keeps old topic when word overlap ≥ 70%", () => {
        ...
        ).toBe("docker compose api setup");
        expect(
            stableTopic("auth refactor api test", "auth refactor api fix"),
        ).toBe("auth refactor api test");
    });

    test("adopts new topic when word overlap < 70%", () => {
        expect(stableTopic("docker setup", "kubernetes deploy")).toBe(
            "kubernetes deploy",
        );
        expect(stableTopic("auth refactor", "ui styling pass")).toBe(
            "ui styling pass",
        );
    });
    ...
    test("ignores punctuation during comparison", () => {
        ...
        expect(stableTopic("auth refactor!", "auth refactor")).toBe(
            "auth refactor!",
        );
        expect(stableTopic("node.js import", "node js import")).toBe(
            "node.js import",
        );
    });
    ...
    test("completely different topics adopt the new one", () => {
        expect(stableTopic("frontend work", "database migration")).toBe(
            "database migration",
        );
    });
});
