...
const TEST_CASES: TestCase[] = JSON.parse(
  fs.readFileSync(path.join(import.meta.dirname, "test-cases-goal.json"), "utf8"),
);
...
function report(results: EvalResult[], model: string): string {
  ...
  for (const r of results) {
    ...
    lines.push(`| 2. Present continuous | ${r.score.presentContinuous ? "✅" : "❌"} |`);
    ...
    lines.push(`| 7. Aligns with expected | ${r.score.alignsWithExpected ? "✅" : "❌"} |`);
    lines.push(`| 8. No trailing period | ${r.score.noTrailingPeriod ? "✅" : "❌"} |`);
...
async function main() {
  ...
  const outPath = path.join(import.meta.dirname, "prompt-eval-goal-report.md");
