    ...
    return new Promise((resolve) => {
        execFile(
            ...
            (err, stdout) => {
                ...
                const untracked = lines.filter((l) =>
                    l.startsWith("??"),
                ).length;
...
export async function fetchStatusline(
    ...
    const branch =
        expectedBranch ||
        (await getBranchFromCwd(cwd)) ||
        readHeadFromFile(cwd);
    ...
    if (!isDegraded()) {
        ...
        if (wtResult) {
            const ttl =
                health.consecutiveWtSuccesses >= 3
                    ? DEFAULT_TTL_MS
                    : DEGRADED_TTL_MS;
...
export function invalidateStatusline(cwd?: string, branch?: string): void {
    if (cwd && branch) {
