...
function makeMockPi() {
  ...
  const commands: { [name: string]: { description: string; handler: (args: string, ctx: any) => Promise<void> } } = {};
  const entries: any[] = [];
...
describe("headingExtension", () => {
  ...
  beforeEach(() => {
    ...
    tmpConfigDir = path.join(os.tmpdir(), `pi-heading-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  test("registers 7 hooks and 3 commands", () => {
    ...
    expect(Object.keys(pi.commands)).toEqual(["heading", "heading-model", "heading-debug"]);
  });
      ...
      branch: [
        {
          type: "custom",
          customType: "heading",
          data: { topic: "Docker", goal: "Fix compose", achievement: "Fixed it" },
        },
      ],
    });
  ...
  test("before_agent_start notifies on summarize error", async () => {
    mockCompleteSimple.mockImplementation(() => Promise.reject(new Error("model down")));
    headingExtension(pi as any);
    const ctx = makeMockCtx();
    pi.handlers["before_agent_start"][0]({ prompt: "help" }, ctx);
    await new Promise((r) => setTimeout(r, 50));
  ...
  test("turn_end skips achievement render when spinner is active", async () => {
    ...
    expect(ctx.notifyCalls.some((n) => n.msg?.includes("skipped-achievement-render"))).toBe(false);
  });
  ...
  test("turn_end notifies on achievement error", async () => {
    ...
    pi.handlers["turn_end"][0]({ message: { content: "done" } }, ctx);
    await new Promise((r) => setTimeout(r, 50));
    expect(ctx.notifyCalls.some((n) => n.msg.includes("Achievement summarize failed"))).toBe(true);
  });
  ...
  test("/heading command sets manual goal and persists", async () => {
    ...
    expect(ctx.widgetCalls.length).toBeGreaterThan(0);
    expect(ctx.notifyCalls.some((n) => n.msg.includes("Heading set"))).toBe(true);
  });
  ...
  test("/heading-model shows models and sets override", async () => {
    ...
    expect(ctx.notifyCalls.some((n) => n.msg.includes("model-b"))).toBe(true);
  });

  test("/heading-model warns when model has no API key", async () => {
    headingExtension(pi as any);
    const models = [{ id: "model-a" }];
    const ctx = makeMockCtx({ models, selectResult: "  model-a", auth: { ok: false } });
    await pi.commands["heading-model"].handler("", ctx);
  ...
  test("/heading-debug clear wipes log", async () => {
    ...
    logDebug({ t: "2026-05-20T12:00:00Z", input: "test", prompt: "p", topicResponse: "", goalResponse: "", rawTopic: "", rawGoal: "", stableTopic: "", finalGoal: "", topicSystemPrompt: "", goalSystemPrompt: "" } as any);
  ...
  test("/heading-debug shows status when log is empty", async () => {
    ...
    const ctx = makeMockCtx();
    await pi.commands["heading-debug"].handler("", ctx);
    expect(ctx.notifyCalls.some((n) => n.msg.includes("No debug entries"))).toBe(true);
  });
});
