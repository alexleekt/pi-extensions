import type { Issue, ParsedContext, ParsedRoadmap, ThemeController } from './types';
...
export const docs = {
   ...
   */
  parseRoadmap(md: string): ParsedRoadmap {
    const sections: ParsedRoadmap = {};
   ...
   */
  parseContext(md: string): ParsedContext {
    const sections: ParsedContext = {};
    const lines = md.split('\n');
    let currentSection: string | null = null;
...
(window as unknown as Record<string, unknown>).docs = docs;
