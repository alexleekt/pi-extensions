...
describe("truncateToWords", () => {
    ...
    test("truncates to maxWords with ellipsis", () => {
        expect(truncateToWords("one two three four five", 3)).toBe(
            "one two three…",
        );
    });
...
describe("cleanLLMOutput", () => {
    ...
    test("strips 'The user wants me to...' prefix", () => {
        expect(
            cleanLLMOutput("The user wants me to validate the results"),
        ).toBe("validate the results");
    });

    test("strips 'The user wants to...' prefix", () => {
        expect(cleanLLMOutput("The user wants to fix the memory leak")).toBe(
            "fix the memory leak",
        );
    });

    test("strips 'The user is...' prefix", () => {
        expect(cleanLLMOutput("The user is asking about Docker setup")).toBe(
            "asking about Docker setup",
        );
    });

    test("strips 'User wants me to...' prefix", () => {
        expect(cleanLLMOutput("User wants me to write a test")).toBe(
            "write a test",
        );
    });
    ...
    test("strips 'Here is the topic:' prefix", () => {
        expect(cleanLLMOutput("Here is the topic: Docker setup")).toBe(
            "Docker setup",
        );
    });
    ...
    test("preserves valid clean output", () => {
        expect(cleanLLMOutput("Docker setup")).toBe("Docker setup");
        expect(cleanLLMOutput("Fix the memory leak")).toBe(
            "Fix the memory leak",
        );
    });
    ...
    test("strips quotes hidden behind prefixes", () => {
        ...
        expect(cleanLLMOutput('The user wants to "Fixed the bug"')).toBe(
            "Fixed the bug",
        );
        expect(cleanLLMOutput('Achievement: "Fixed the bug"')).toBe(
            "Fixed the bug",
        );
        expect(cleanLLMOutput('The user wants me to "Deploy to K8s"')).toBe(
            "Deploy to K8s",
        );
    });
    ...
    test("handles malformed JSON missing closing brace", () => {
        ...
        expect(cleanLLMOutput('{"result": "Fixed the bug"')).toBe(
...
describe("readPromptFile", () => {
    ...
    beforeEach(() => {
        ...
        tmpDefaultDir = path.join(
            os.tmpdir(),
            `pi-heading-default-${Date.now()}`,
        );
    ...
    test("falls back to default prompt when user missing", () => {
        fs.writeFileSync(
            path.join(tmpDefaultDir, "topic.md"),
