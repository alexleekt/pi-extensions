...
export default function SingleSelect({ payload }: SingleSelectProps) {
	...
	const [previewOption, setPreviewOption] = useState<{ title: string; description?: string } | null>(null);
	...
	}, [filtered, activeIndex, showComment, sendResult]);
		...
		<div className="flex h-screen flex-col">
			...
			<div className="flex flex-1 overflow-hidden">
				...
				{previewOption?.description && (
					<div className="w-80 overflow-y-auto border-l border-border bg-muted/30 p-4">
						<h2 className="mb-2 text-sm font-semibold text-muted-foreground">Details</h2>
						<div className="text-sm">{previewOption.description}</div>
					</div>
				)}
			</div>
