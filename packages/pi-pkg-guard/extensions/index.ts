...
export function isPiSettings(value: unknown): value is PiSettings {
    ...
    if (candidate.packages !== undefined) {
        ...
        if (!candidate.packages.every((p) => typeof p === "string"))
            return false;
    }

    if (candidate.extensions !== undefined) {
        if (!Array.isArray(candidate.extensions)) return false;
        if (!candidate.extensions.every((e) => typeof e === "string"))
            return false;
    }
...
export function isPackageSnapshot(value: unknown): value is PackageSnapshot {
    ...
    if (v.excludedPackages !== undefined) {
        ...
        if (!v.excludedPackages.every((p) => typeof p === "string"))
            return false;
    }
...
export function validatePackageSnapshot(data: unknown): ValidationResult {
    ...
    if (candidate.$schema === undefined) {
        ...
        const isValidSchemaUrl = ALLOWED_SNAPSHOT_SCHEMA_PATTERNS.some(
            (pattern) =>
    ...
    if (candidate.timestamp === undefined) {
        ...
        errors.push(
            "Field 'timestamp' must be a valid ISO 8601 date-time string",
        );
    }
        ...
        if (!Array.isArray(candidate.excludedPackages)) {
            ...
            for (let i = 0; i < candidate.excludedPackages.length; i++) {
                if (typeof candidate.excludedPackages[i] !== "string") {
                    errors.push(
                        `Field 'excludedPackages[${i}]' must be a string`,
                    );
                }
            }
        }
...
export function writeExtensionSettings(config: ExtensionSettings): void {
    try {
        ...
        console.error(
            "[pi-pkg-guard] Failed to write extension settings:",
            error,
        );
    }
}
...
export function registerPackages(status: PackageStatus): void {
    ...
    const existingPackages = new Set(
        settings.packages.map(normalizePackageName),
    );
...
export async function cleanupOrphanedPackages(): Promise<string[]> {
    ...
    const npmPackages = settings.packages.filter((p) =>
        p.startsWith(NPM_PREFIX),
    );
...
export async function syncGistBackup(
    ...
    if (!isValidGistId(gistId)) {
        return {
            success: false,
            error: "Invalid gist ID format. Gist IDs must be hexadecimal characters only.",
        };
    }
    ...
    if (!isGhInstalled()) {
        return {
            success: false,
            error: "GitHub CLI (gh) is not installed. Install it from https://cli.github.com or use the local backup only.",
        };
    }
...
export async function createGist(): Promise<{
    ...
    try {
        ...
        return {
            success: false,
            error: "Could not extract gist ID from output",
        };
    } catch (error) {
        return {
            success: false,
                    ...
                    action: async () => {
                        ...
                        if (gistInput !== undefined) {
                            ...
                            config.gistId =
                                trimmed === ""
                                    ? undefined
                                    : extractGistId(trimmed);
                            config.gistEnabled = config.gistId
                                ? true
                                : undefined;
                            writeExtensionSettings(config);
                            ctx.ui.notify(
            ...
            await runMenuLoop(
                ...
                () => {
                    ...
                    const gistDisplay = config.gistId
                        ...
                        : "☁️ not configured";
                    const syncDisplay =
                        config.gistEnabled === false ? "⏸️ off" : "⏳ on";
                    const pathDisplay =
                        currentPath === DEFAULT_BACKUP_PATH
                            ? "local"
                            : "custom";
