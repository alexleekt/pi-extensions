    ...
    useEffect(() => {
        ...
        if (firstUnanswered) {
            ...
            if (el) {
                ...
                if (!hasOptions) {
                    const textarea = optionRefs.current.get(
                        `${firstUnanswered.title}-freeform`,
                    );
                    textarea?.focus();
                }
            }
    ...
    const toggleMultiAnswer = (questionTitle: string, optionTitle: string) => {
        setAnswers((prev) => {
            ...
            if (arr.includes(optionTitle)) {
                return {
                    ...prev,
                    [questionTitle]: arr.filter((v) => v !== optionTitle),
                };
            }
                                        ...
                                        {q.allowMultiple
                                            ? q.options.map((opt) => {
                                                  ...
                                                  return (
                                                      ...
                                                      >
                                                          <div
                                                              className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded ${isSelected ? "bg-primary text-primary-foreground" : "border border-border"}`}
                                                          >
                                                              {isSelected && (
                                                                  <CheckIcon
                                                                      checked={
