...
export async function runSuite<T extends TestCase>(
  ...
  const { testCases, promptBuilder, extractMode, scorers, modelConfig } = suite;
  ...
  for (let i = 0; i < testCases.length; i++) {
    ...
    try {
      ...
      console.log(`${status} "${extracted.slice(0, 60)}" (${latencyMs}ms)`);
    } catch (err) {
      const latencyMs = Math.round(performance.now() - start);
...
export function generateReport<T extends TestCase>(
  ...
  lines.push(`|--------|-------|`);
  lines.push(`| Pass rate | ${passRate.toFixed(1)}% (${passed}/${results.length}) |`);
  lines.push(`| Avg score | ${avgScore.toFixed(1)}% (${totalScore}/${totalMax}) |`);
  lines.push(`| Total latency | ${results.reduce((s, r) => s + r.latencyMs, 0)}ms |`);
  lines.push("");
  ...
  for (const r of results) {
    ...
    lines.push("");
    lines.push(`**Score**: ${r.score.total}/${r.score.max} | **Latency**: ${r.latencyMs}ms`);
    lines.push("");
    lines.push("```text");
    lines.push(r.raw);
    lines.push("```");
    ...
    if (r.score.results.length > 0) {
      ...
      for (const sr of r.score.results) {
        ...
        const detail = sr.detail ?? (sr.value !== undefined ? String(sr.value) : "");
...
export const scorers = {
  ...
  pastTense(): Scorer {
    ...
    return ({ text }) => {
      ...
      const passed = pastTenseStarters.test(trimmed) || pastTenseWords.test(trimmed);
      return {
        name: "pastTense",
        passed,
      };
  ...
  alignsWithExpected(fieldName: string, threshold = 0.5): Scorer {
    return ({ text, testCase }) => {
      ...
      if (!expected) {
        return { name: `alignsWithExpected (${fieldName})`, passed: true };
      }
      const normA = expected.toLowerCase().replace(/[^a-z0-9\s]/g, "").split(/\s+/).filter(Boolean);
      const normB = text.toLowerCase().replace(/[^a-z0-9\s]/g, "").split(/\s+/).filter(Boolean);
      if (normA.length === 0) {
        return { name: `alignsWithExpected (${fieldName})`, passed: true };
      }
...
export interface OptimizeResult {
  ...
  allIterations: Array<{ iteration: number; passRate: number; promptText: string }>;
}
...
export async function optimizeSuite<T extends TestCase>(
  ...
  for (let i = 1; i <= maxIterations; i++) {
    ...
    if (passRate >= targetPassRate) {
      ...
      return { success: true, bestIteration: best, allIterations: iterations };
    }

    // Build critic prompt from failures
    const failures = results.filter((r) => r.score.total < r.score.max);
    const failureSummary = failures
      .map((r) => `Input: "${r.testCase.input}" → Got: "${r.extracted}" (score ${r.score.total}/${r.score.max})`)
      .join("\n");
    ...
    try {
      const improved = await callLLM(criticSystem, "Rewrite the prompt to fix the failures.", criticModel, {
        maxTokens: 512,
        temperature: 0.3,
      });
