...
export function loadPrompt(fileName: string, customPath?: string): string {
    const p =
        customPath ?? path.join(__dirname, "..", "..", "prompts", fileName);
    return fs
        .readFileSync(p, "utf8")
        .replace(/^---\s*\n[\s\S]*?---\s*(?:\n|$)/, "");
}
...
function buildPrompt(instructions: string, maxWords: number, message: string) {
    ...
    return {
        ...
        maxTokens: Math.min(128, maxWords * 2 + 8),
    };
}

function makeSuite(
    testCases: GoalTestCase[],
    instructions: string,
): EvalSuite<GoalTestCase> {
    const suiteScorers = [
    ...
    return {
        ...
        testCases,
        promptBuilder: (tc) =>
            buildPrompt(instructions, tc.maxGoalWords, tc.input),
        extractMode: "json",
...
export function suiteFactory(
    testCasesFile = "test-cases.json",
): (promptText: string) => EvalSuite<GoalTestCase> {
    const testCases = loadTestCases(testCasesFile);
