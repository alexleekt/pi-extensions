...
| `glimpseui` (peer dep) | `node_modules/glimpseui/src/glimpse` | `npm run build:macos` in that worktree | `ask_user` returns `"No UI available..."` because `glimpseui.prompt()` can't spawn the native binary |
